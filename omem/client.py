...
def _error_class_for_status(status: int) -> type[OmemHttpError]:
    if status == 401:
        return OmemAuthError
    if status == 403:
        return OmemForbiddenError
    if status == 402:
        return OmemQuotaExceededError
    if status == 429:
        return OmemRateLimitError
    if status == 413:
        return OmemPayloadTooLargeError
    if status == 400:
        return OmemValidationError
    if status >= 500:
        return OmemServerError
    return OmemHttpError


def _unwrap_api_payload(payload: Any, *, request_id: Optional[str]) -> Dict[str, Any]:
    if not isinstance(payload, dict):
        raise OmemClientError("invalid_json_response: expected object")
