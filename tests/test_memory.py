...
import omem
from omem.client import MemoryClient, OmemClientError, OmemHttpError
from omem.memory import DEFAULT_ENDPOINT, Conversation, Memory
from omem.models import AddResult, MemoryItem, SearchResult
...
class TestPackageMetadata:
    def test_public_metadata_points_to_omnimemory_repository(self):
        root = Path(__file__).resolve().parents[1]
        pyproject = (root / "pyproject.toml").read_text(encoding="utf-8")
        init_py = (root / "omem" / "__init__.py").read_text(encoding="utf-8")

        assert "https://github.com/Omnimemory/python-sdk" in pyproject
        assert "https://github.com/Omnimemory/python-sdk" in init_py
    ...
    def test_packaged_modules_import_without_internal_core_dependencies(self):
        for module in pkgutil.walk_packages(omem.__path__, prefix="omem."):
...
class TestPublicApiBoundary:
    def test_package_exports_only_supported_public_api(self):
        assert set(omem.__all__) == PUBLIC_EXPORTS
    ...
    def test_removed_graph_and_session_symbols_are_not_public(self):
        for name in (
            ...
            "EventContext",
            "ExtractedKnowledge",
            "OmemUnsupportedFeatureError",
        ):
            assert not hasattr(omem, name)
...
class TestMemoryInit:
    ...
    def test_defaults_to_public_v2_endpoint_and_saas_auth(self, mock_client_cls):
        ...
        assert DEFAULT_ENDPOINT == "https://api.omnimemory.cn/api/v2/memory"
...
class TestMemoryClientHttp:
    ...
    def test_raises_for_unsuccessful_v2_envelope_even_when_http_200(self):
        ...
        with pytest.raises(OmemHttpError) as exc:
